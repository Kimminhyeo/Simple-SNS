...
public class PostControllerTest {
    ...
    void 포스트수정시_데이터베이스_에러_발생시_에러발생() throws Exception {
        doThrow(new SnsApplicationException(ErrorCode.INTERNAL_SERVER_ERROR)).when(postService).modify(eq("title"), eq("body"), any(), eq(1));
        mockMvc.perform(put("/api/v1/posts/1")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(new PostModifyRequest("title", "body"))))
                .andDo(print())
    ...
    void 포스트삭제시_데이터베이스_에러_발생시_에러발생() throws Exception {
        doThrow(new SnsApplicationException(ErrorCode.INTERNAL_SERVER_ERROR)).when(postService).delete(any(), eq(1));
        mockMvc.perform(delete("/api/v1/posts/1")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().is(ErrorCode.INTERNAL_SERVER_ERROR.getStatus().value()));
